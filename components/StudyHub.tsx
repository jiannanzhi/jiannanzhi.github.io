...
const StudyHub: React.FC<StudyHubProps> = ({
  ...
  const handleOpenHighlights = async () => {
    ...
    if (highlightGroups.length === 0) {
      ...
      try {
        const groups = await buildHighlightGroupsForBooks(boundBooks);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleOpenHighlights}
                className={`flex items-center gap-1 px-3 py-2 rounded-xl text-sm transition-all active:scale-95 ${btnClass} ${showHighlights ? 'text-rose-400' : subTextClass}`}
              >
